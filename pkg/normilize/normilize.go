...
func Days(rule string) (int, error) {
...
func DaysWeek(rule string) ([]int, error) {
	var week []int
	parts := strings.Split(rule, " ")
	if len(parts) != 2 {
		err := errors.New("invalid input data value")
...
func Months(rule string) ([]int, error) {
...
func DaysForMonth(now time.Time, rule string) ([]int, error) {
	var daysForSort []int
...
func SearchTasks(searchLine string) string {
	fmt.Println(searchLine)
	searchDate, err := time.Parse("02.01.2006", searchLine)
	if err != nil {
		log.Println("In searchLine not date")
		return ""
	}
	dateToFind := searchDate.Format("20060102")
	return dateToFind
}